...
## 5. PTW
硬件页表查询page table walker
> wsong83:<br>
> 我现在的理解硬件PTW其实就是简单的状态机去搜索页表，其实没设么特别的设计。这一节先放这里，也许去掉。<br>
> leishangwen:<br>
> 虽然不特别，但是PTW也是Rocket的一部分，建议结合Rocket讲，给出Rocket的PTW的设计，比如：接口信号、执行过程、内部结构等，

## 6. FPU
> wsong83:<br>
> 这个超过我的能力范围，需要牛人来讲，看看有谁懂IEEE浮点标准还懂硬件实现。也许bitmain的王逵。<br>
> leishangwen:<br>
> FPU应该不是Rocket的一部分吧？


## 7. Rocket处理器RoCC设计分析（leishangwen）
...
>
> wsong83:<br>
> 看来真的是解码。这个部分我觉得最好在2.2来讲。我在思考可不可以拿RV64I和AArch64的编码来比一比，应该一下子就清楚了。
> 用电路说其实较难说明白，在实现上其实都是列张大表然后硬件去查表（case操作）。
> 这一节建议去掉。<br>
> leishangwen:<br>
> RISC-V的优势除了open，应该还有其他优势，比如在指令结构的设计上，而指令结构设计的优势就体现在能更好的实现解码、实现指令功能，所以这里说要对Rocket的指令解码过程分析，结合具体的处理器实现，更能讲的明白。当然，具体细节我不了解，还需要明白的人来写。如果实在不好结合Rocket讲RISC-V的优势，那么就放在2.2节吧。
